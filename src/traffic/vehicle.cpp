...
Vehicle::Vehicle(SDL_Renderer *renderer, int vehicle_id, LaneId startLane,
    ...
    : m_renderer(renderer), m_id(vehicle_id), m_position(startPos), m_velocity(0.0F, 0.0F), m_status(VehicleStatus::MOVING), m_currentLane(startLane), m_targetLane(startLane), m_facing(facing), m_turnIntent(TurnBehaviour::STRAIGHT), m_waitTime(0.0F), m_isPriority(false)
{
  checkPriority();
}
